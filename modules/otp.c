...
uint32_t simple_mktime(int year, int month, int day, int hour, int minute,
		       int second)
{
...
static void clock_event(enum sys_message msg)
{
     ...
     // Calculate timestamp
     uint32_t time =
	  simple_mktime(rtca_time.year, rtca_time.mon - 1, rtca_time.day,
...
{
     enum sys_message ev_type =
	  activated ? SYS_MSG_RTC_SECOND : SYS_MSG_NONE;
#if defined(CONFIG_MOD_OTP_SOUND_CUE)
     otp_first_code = 1;
#endif
...
void mod_otp_init()
{
     menu_add_entry("OTP", &otp_gen_next,	/* up         */
		    &otp_gen_prev,	/* down       */
#if defined(CONFIG_MOD_OTP_SOUND_CUE)
		    &otp_toggle_beep,	/* num        */
#else
